...
public class LittleSearchEngine {
	...
	throws FileNotFoundException {
		HashMap<String, Occurrence> hm = new HashMap<String, Occurrence>(1000,2.0f);
		Scanner sci = new Scanner(new File(docFile));
		while(sci.hasNext()){
			String nu = sci.next(); nu = getKeyWord(nu);
			if(nu!=null){
				if(hm.containsKey(nu)){
					Occurrence ge = hm.get(nu);
					ge.frequency++;
					hm.put(nu, ge);
				}
				else{
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		for(int p = 0; p < listOne.size(); p++)
		{
			if(finishedList.size() <= 4)
			{
				...
				for(int w = 0; w < listTwo.size(); w++)
				{
					...
					if(L2 <= L1)
					{
						if(!finishedList.contains(docNameOne) && finishedList.size() <= 4)
						{
							finishedList.add(docNameOne);
						}
					}
					else if(L2 > L1)
					{
						if(!finishedList.contains(docNameTwo) && finishedList.size() <= 4)
						{
							finishedList.add(docNameTwo);
						}
					}
				}
			}
		...
		for(int k = 0; k < finishedList.size(); k++)
		{
			if(k + 1 == finishedList.size())
				System.out.print(finishedList.get(k));
			else
				System.out.print(finishedList.get(k) + ", ");
		} 
